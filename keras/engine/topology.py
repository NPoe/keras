...
class Layer(object):
    ...
    def __init__(self, **kwargs):
        ...
        if 'weights' in kwargs:
            self._set_initial_weights(kwargs['weights'])
        else:
            self._set_initial_weights(None)


    def _set_initial_weights(self, weights = None):
    ...
    def _make_lrp_backwards_function(self, mask = None):
        raise NotImplementedError("LRP backwards function not implemented for layer", self.name)

    def _make_lrp_forwards_function(self, mask = None):
        self.lrp_forwards_function = K.function([self.input], [self.output])
